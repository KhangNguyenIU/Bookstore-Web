...
const UserPageAdmin = (props) => {
    ...
    const initUser = () => {
        ...
        getUserLikedBook(id).then(response => {
            if (response.error) {
                console.log(response.error);
            } else {
            ...
            <div className="row content">
                <div className="col-sm-12">
                    <div className="text-center">
                        <Avatar src={user.photo} style={{ width: "150px", height: "150px", margin: '20px auto' }} />
                        <p className="custom-heading m-0">{user.username}</p>
                        <p className="custom-text m-0">{user.email}</p>
                    </div>
                </div>
            </div>
        </div>
